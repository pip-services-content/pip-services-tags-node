# Configuration Guide <br/> Tags Microservice
...
Example to retrieve microservice configuration remote repository by key 'pip-services-tags':
```javascript
{
    "config": {
        "type": "remote",
        ...
        "key": "pip-services-tags" // Microservice config key
    } 
    ...
}
```
...
- path: string - file path where Tags objects are stored. The object are written into the file in JSON format.
- data: [Tags] - (optional) array of Tags objects which is used to initialize the dataset. This option is only used in testing to set a known state on test start.
...
However, Tags microservice doesn't have any dependencies. So, this section should be omitted entirely or kept empty.
...
The **Api** section deines configuration of the microservice services (also called endpoints) to provide API for the consumers. 
